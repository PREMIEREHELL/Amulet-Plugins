...
class SetFrames(wx.Panel, DefaultOperationUI):
    ...
    def imageP(self, imagedata):
        ...
        self.box.Add(self.grid2, 0, wx.LEFT , 5)
        self.box.Add(self.grid1, 0,wx.LEFT | wx.BOTTOM , 5)
    ...
    def exportImg(self, _):
        ...
        if len(self.ch) > 0:
            for x in self.ch:
                print( cMaps[x]['mapsD']['colors'])
                save = wx.Bitmap.FromBufferRGBA(128, 128, cMaps[x]['mapsD']['colors'].tostring())
                save.SaveFile(dirname+"\\"+str(x)+".png",
                    wx.BITMAP_TYPE_PNG)


        # for x in imgs.get("mapsD"):
        #     print(x["mapId"])
        # save = wx.Bitmap.FromBufferRGBA(128,128,imgs.get("mapsD")[0]["colors"].tostring())
        # save.SaveFile(r"C:\Users\DJ\AppData\Local\Packages\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\LocalCache\local-packages\Python39\site-packages\amulet_map_editor\programs\edit\plugins\operations\stock_plugins\operations\test.png", wx.BITMAP_TYPE_PNG)
    ...
    def setSelMaps(self, _):
       ...
       # print(self._mapSL.GetSelection())
    ...
    def _run_get_sdata(self):
        ...
        for cnt, x in enumerate(self._mapSL.GetItems()):
           # print(self._mapList.GetSelections())
...
export = dict(name="#A_Image to Frames, Plugin V2.16b", operation=SetFrames)  # by PreimereHell
