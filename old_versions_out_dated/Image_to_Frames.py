...
class SetFrames(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        # self._l_map.SetLabel("First:")
        # self.bottom_sizer.Add(self._l_map)
        ...
        self._mapList = wx.ListBox(self, style=wx.LB_MULTIPLE, size=(70,65), choices=self._run_get_slist)
        self._mapList.Bind(wx.EVT_LISTBOX, self.onFocus)
        self._custommapList = wx.ListBox(self, style=wx.LB_SINGLE, size=(70, 65), choices=list(currentMaps.keys()))
        ...
        self.right_sizer.Add(self._run_setImData_button, 0, wx.RIGHT | wx.LEFT , 20)
        ...
        self.dis_pnt = wx.BoxSizer(wx.HORIZONTAL)
        ...
        self.dis_pnt.Add(self._mode_description, 5, wx.TOP | wx.RIGHT, 0)
        self.dis_pnt.Add(self.boxgrid)
        self.range = wx.TextCtrl(
            self, style=wx.TE_LEFT | wx.TE_BESTWRAP, size=(90, 20)
        )
        self.range_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self._l_map = wx.StaticText(self, wx.LEFT)
        self._l_map.SetLabel("_ SET THE ASPECT\n RATIO HERE N_N:")
        self.range.SetValue("5_0")
        ...
        self.bottom_sizer.Add(self.radio_frameC, 20, wx.LEFT, 2)
    ...
    def imageP(self, imagedata):
        ...
        maps, c, r, mips = imagedata.get("maps"),int(imagedata.get("col")),int(imagedata.get("row")),imagedata.get('mapsD')
        #Maps = maps
        id = self.idCount
        self.idCount += 1
        self._direct = wx.ListBox(self, size=(70,65),choices=list(facing.keys()), id=id+929)
       ...
       # print(maps)
        ...
        self.delSelf = wx.Button(self, size=(30,20),label="Hide", name=str(c)+"_"+str(r), id=id)
        ...
        self.box.Fit(self)
        for x in mips:
            size = 30#(128/(len(imagedata)-startP) + (len(imagedata)-startP)/8)
            self.img = Image.fromarray(np.reshape(x['colors'], (128, 128, 4)), 'RGBA')
            self.imm = wx.StaticBitmap(self, bitmap=scale_bitmap(wx.Bitmap.FromBufferRGBA(128, 128, self.img.tobytes()), size, size))
            self.grid.Add(self.imm, wx.TOP )

        self.box.Add(self.grid,0, wx.LEFT | wx.TOP,0)

        self.grid1.Add(self.delSelf, 0, wx.LEFT | wx.TOP, 0)
        self.grid1.Add(self.AddSelf, 0, wx.LEFT | wx.TOP, 0)

        self.box.Add(self.grid2, 0, wx.TOP)
        self.box.Add(self.grid1, 0, wx.LEFT | wx.TOP, 0)
        self.grid.Fit(self)
        self.bottomImage_sizer.Add(self.box, 0, wx.LEFT, 0)
    ...
    def onAddPicture(self, event):
        ...
        pntd = 0
        faced = 0
        if self._mode_description.GetValue() != "" and self.mapveiwing != 1:
            theBlockData = self.jgeterseter()
        for x in txtCtrls:
            if x.GetId() == id+777:
                uuid =  x.GetValue().replace('[','').replace(']','').replace('"','').replace(' ','').replace('\'','').split(',')
        for lst in lstCtrls:
            if lst.GetId() == id:
            ...
            if btn.GetId() == id:
                ...
                if btn.GetLabel() == "Set":
                     self.boxSetter(int(uuid[0]), rot, "Barrel", "barrel", dim,
                                    fac, pntd)
                if pntd == 0 and fac == 0:
                    self.canvas.renderer.render_world._resource_pack
                    self.canvas.camera.set_rotation([(-70) + (cx / cz), 60.0])
                    sg = SelectionGroup(SelectionBox((cx, cy - 4, cz), (cx + int(dim[1]), cy - 3, cz + int(dim[0]))))
                if pntd == 1 and fac == 0:
                    self.canvas.camera.set_rotation([(-70) + (cx / cz), 60.0])
                    sg = SelectionGroup(SelectionBox((cx, cy - 4, cz), (cx + int(dim[0]), cy - 3, cz + int(dim[1]))))
                if pntd == 2 and fac == 0:
                    self.canvas.camera.set_rotation([(-70) + (cx / cz), 60.0])
                    sg = SelectionGroup(SelectionBox((cx + int(dim[1]), cy - 4, cz + int(dim[0])),(cx, cy - 3, cz)))
                if pntd == 3 and fac == 0:
                    self.canvas.camera.set_rotation([(-70) + (cx / cz), 60.0])
                    sg = SelectionGroup(SelectionBox((cx + int(dim[0]), cy - 4, cz + int(dim[1])),(cx, cy - 3, cz)))
                ...
                if pntd >= 0 and pntd <= 1 and fac >= 2:
                    self.canvas.camera.set_rotation([(-170) + (cz / cx), -10.0])
                    czz = cz-4
                    czz +=1
                    sg = SelectionGroup(SelectionBox((cx, cy, cz - 4), (cx + int(dim[1]), cy + int(dim[0]), czz)))
                if pntd >= 2 and fac >= 2:
                    czz = cz-4
                    czz += 1
                    self.canvas.camera.set_rotation([(-170) + (cz / cx), -10.0])
                    sg = SelectionGroup(SelectionBox((cx, cy, cz - 4), (cx + int(dim[0]), cy + int(dim[1]), czz )))

                if pntd >= 0 and pntd <= 1 and fac >= 4:
                    cxx = cx - 5
                    cxx += 1
                    self.canvas.camera.set_rotation([(70) + (cz / cx), -10.0])
                    sg = SelectionGroup(SelectionBox((cx - 5, cy, cz), (cxx, cy + int(dim[0]), cz + int(dim[1]))))
                if pntd >= 2 and fac >= 4:
                    cxx = cx - 5
                    cxx += 1
                    self.canvas.camera.set_rotation([(70) + (cz / cx), -10.0])
                    sg = SelectionGroup(SelectionBox((cx - 5, cy, cz), (cxx, cy + int(dim[1]), cz + int(dim[0]))))

                if btn.GetLabel() != "Set" and btn.GetLabel() != "Hide":
                    self.canvas.selection.set_selection_group(sg)
                    btn.SetLabel("Set")
                self.canvas.camera.set_location([cx, 100.0, cz])
    ...
    def onFocusCmap(self, evt):
        ...
        for x in setdata:
            sizex = currentMaps[x]["row"]
            sizey = currentMaps[x]["col"]

            for i in range(0, len(currentMaps[x]["maps"])):
                currentMaps[x]["maps"][i] = int(currentMaps[x]["maps"][i])

            currentMaps[x]["maps"].sort()
            # for m in currentMaps[x]["maps"]:
            #     st = "map_" + str(m)
            #     Maps.append(m)
            #     enS = str(st).encode("utf-8")
            #     map = self.world.level_wrapper._level_manager._db.get(enS)
            #     data = amulet_nbt.load(map, little_endian=True)
            #     data2.append(np.reshape(data.get('colors'), (128, 128, 4)))
        #self.imageP(data2,int(sizey) , int(sizex), Maps, 0)
        self.imageP(currentMaps[x])
    ...
    def _run_get_sdata(self):
      ...
      #  print(MapDic)
        ...
        size = self.range.GetValue().split("_")
    ...
    def _run_import(self, _):
     ...
     #   print(self.customMaps)
        ...
        for x in range(0, len(imgs)):
            ...
            #rawMap = Map.save_to(compressed=False, little_endian=True)
            #MapPV.append(np.reshape(Map.get('colors'), (128,128,4)))
            #mapData[uuid] = Map
    ...
    def jgeterseter(self):
        theBlockData = json.loads(self._mode_description.GetValue())
        return theBlockData
    ...
    def boxSetter(self, uuid, fRotation, ItemId, ID, box_dim,facing,pntd):
        ...
        if facing == 0 and pntd == 0:
            xyz = self.xyzOrder(fx, fy, fz); fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 315; blocks["xyz"] = fx, fy, fz
        if facing == 0 and pntd == 1:
            xyz = self.xyzOrder(fz, fy, fx); fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fx.reverse();fRotation = 180;blocks["xyz"] = fz, fy, fx
        if facing == 0 and pntd == 2:
            xyz = self.xyzOrder(fx, fy, fz);fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 225;fx.reverse();fz.reverse();blocks["xyz"] = fx, fy, fz
        if facing == 0 and pntd == 3:
            xyz = self.xyzOrder(fz, fy, fx);fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 270;fz.reverse(); blocks["xyz"] = fz, fy, fx
        if facing == 1 and pntd == 0:
            xyz = self.xyzOrder(fx, fy, fz);fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 315; fz.reverse(); blocks["xyz"] = fx, fy, fz
        if facing == 1 and pntd == 1:
            xyz = self.xyzOrder(fz, fy, fx);fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fx.reverse();fz.reverse();fRotation = 270; blocks["xyz"] = fz, fy, fx
        if facing == 1 and pntd == 2:
            xyz = self.xyzOrder(fx, fy, fz); fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 225;fx.reverse();blocks["xyz"] = fx, fy, fz
        if facing == 1 and pntd == 3:
            xyz = self.xyzOrder(fz, fy, fx);fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 0;blocks["xyz"] = fz, fy, fx
        if facing == 2 and pntd == 0:
            xyz = self.xyzOrder(fx, fy, fz); fx, fy, fz = [], [], []
            for x, y, z in xyz:
                fx.append(x), fy.append(y), fz.append(z)
         #       print(x,y,z)
            fy.reverse();fRotation = 225;blocks["xyz"] = fx, fy, fz
        if facing == 2 and pntd == 1:
            xyz = self.xyzOrder(fx, fy, fz);fx, fy, fz = [], [], []
            for x, y, z in xyz:
                fx.append(x), fy.append(y), fz.append(z)
            fRotation = 315;fx.reverse();blocks["xyz"] = fx, fy, fz
        if facing == 2 and pntd == 2:
            xyz = self.xyzOrder(fy, fx, fz);fx, fy, fz = [], [], []
            for x, y, z in xyz:
                fx.append(x), fy.append(y), fz.append(z)
            fRotation = 180;fy.reverse();fx.reverse();blocks["xyz"] = fy, fx, fz
        ...
        if facing == 3 and pntd == 0:
            ...
            for x, y, z in xyz:
                fx.append(x), fy.append(y), fz.append(z)
            fRotation = 45; fx.reverse(); fy.reverse(); blocks["xyz"] = fx, fy, fz
        ...
        if facing == 4 and pntd == 0:
            xyz = self.xyzOrder(fx, fz, fy);fx, fy, fz = [], [], []
            for x, y, z in xyz:
                fx.append(x), fy.append(y), fz.append(z)
            fz.reverse();fy.reverse();fRotation = 225;blocks["xyz"] = fx, fz, fy
        if facing == 4 and pntd == 1:
            xyz = self.xyzOrder(fx, fz, fy);  fx, fy, fz = [], [], [];
            for x, y, z in xyz:
                fx.append(x), fy.append(y), fz.append(z)
            fRotation = 315; blocks["xyz"] = fx, fz, fy
        ...
        if facing == 4 and pntd == 3:
            ...
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 270;fz.reverse();fy.reverse(); blocks["xyz"] = fy, fx, fz

        if facing == 5 and pntd == 0:
            xyz = self.xyzOrder(fx, fz, fy);fx, fy, fz = [], [], [];#print("EAST 0")
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 45;fz.reverse();blocks["xyz"] = fx, fz, fy
        if facing == 5 and pntd == 1:
            xyz = self.xyzOrder(fx, fz, fy);fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fy.reverse();fRotation = 315;blocks["xyz"] = fx, fz, fy

        if facing == 5 and pntd == 2:
            xyz = self.xyzOrder(fy, fx, fz); fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fz.reverse();fy.reverse();fRotation = 180;  fy.reverse(); fx.reverse(); blocks["xyz"] = fy, fx, fz
        if facing == 5 and pntd == 3:
            xyz = self.xyzOrder(fy, fx, fz); fx, fy, fz = [], [], []
            for x, y, z  in xyz:
                fx.append(x), fy.append(y),fz.append(z)
            fRotation = 270; blocks["xyz"] = fy, fx, fz

        for b in range(0,len(blocks["xyz"][0])):
            x , y , z = blocks["xyz"][0][b],blocks["xyz"][1][b],blocks["xyz"][2][b]

           # print(x , y , z, "LAST FOR LOOP" , uuid)
            Snbt = TAG_Compound({
            ...
            self._mode_description.SetValue(str(theBlockData).replace('\'', '"'))
            self.thePCount += 1
            uuid += 1
            self._run_text.SetValue("map_" + str(uuid))

        self.canvas.run_operation(lambda: self._refresh_chunk(self.canvas.dimension, self.world, self.canvas.selection.selection_group.min_x, self.canvas.selection.selection_group.min_z))
    ...
    def Finisher(self, ID, BE):
        ...
        for x in theBlockData:
            ...
            # print( x , y, z, uuid, facing, fRotation, pz,pz )
            Snbt = TAG_Compound({
                "facing_direction": TAG_Int(facing),
                "item_frame_map_bit": TAG_Byte(1)
            })
    ...
    def cmapDic(self, dic):
        ...
        for x in dic:
            ...
            currentMaps[dic[x]["name"]]["maps"].append(dic[x]["map"].replace('b','').replace("\'","").replace('map_',''))
...
export = dict(name="#A_Image to Frames, Plugin V1.8b", operation=SetFrames) #by PreimereHell
