...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._run_getSData_button = wx.Button(self, label="Get Structure Data")
        ...
        self._run_text.SetValue("Name Structure Here")
        self._run_setSData_button = wx.Button(self, label="Save Structure \n Data to world ")
        ...
        self._run_setLData_button = wx.Button(self, label="Load MC \n Stucture File")
        self._run_setLData_button.Bind(wx.EVT_BUTTON, self._run_load)
        bottom_sizer.Add(self._run_setLData_button, 0,   wx.LEFT , 0)

        self._run_Del_button = wx.Button(self, label="DELETE Structure \n from World")
    ...
    def _run_get_sdata(self, _):
        ...
        self._structlist.Append(setdata)
        # for p in player:
        #     print(p)

    def _run_set_sdata(self, _):  # b'structuretemplate_mystructure:test'
        if 'structuretemplate_mystructure:' not in self._run_text.GetValue():
            fixKey = "structuretemplate_mystructure:" + self._run_text.GetValue()
    ...
    def _run_get_slist(self):
        world = self.world.level_wrapper._level_manager._db.keys()
        currentStructure = []
        currentStructure.append("Structures In this World")
        for w in world:
            if b'\xff' not in w:
                if b'\x00' not in w:
                    if b'structuretemplate_mystructure' in w:
...
export = dict(name="Edit / Embed Mc Structure", operation=SetBlock)
