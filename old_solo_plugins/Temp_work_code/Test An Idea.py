            ...
            if self.world.level_wrapper.version >= (1, 18, 30, 4, 0):
                ...
                for lk_data in chunk_dict.keys():
                    ...
                    for ent_data in chunk_dict[lk_data]:
                        ...
                        ent_data['internalComponents']['EntityStorageKeyComponent'][
                            'StorageKey'] = amulet_nbt.TAG_String(
            ...
            if copy:
                ...
                if self.world.level_wrapper.version >= (1, 18, 30, 4, 0):
                    ...
                    if (cx, cz) != (nx, nz):
                        ...
                        old_raw_keep = []
                        while point < max:
                            data_old, p = Nbt.load(old_raw[b'2'][point:], compressed=False, little_endian=True,
                                                   offset=True)
                            point += p
    ...
    def nbt_loder(self, raw):
        try:
            new_raw = Nbt.load(raw.replace(b'\x08\n\x00StorageKey\x08\x00',
                                           b'\x07\n\x00StorageKey\x08\x00\x00\x00'), compressed=False,
                               little_endian=True)
        except:
        ...
        for nbt_data in self.found_entities:
            ...
            if (x, y, z) in selection:
                ...
                nbt_pos = amulet_nbt.TAG_List([amulet_nbt.TAG_Double(sum([new_pos[0],
                                                                          math.modf(abs(nbt_data.get("Pos")[0].value))[
                                                                              0]])),
                                               amulet_nbt.TAG_Double(sum([new_pos[1],
                                                                          math.modf(abs(nbt_data.get("Pos")[1].value))[
                                                                              0]])),
                                               amulet_nbt.TAG_Double(sum([new_pos[2],
                                                                          math.modf(abs(nbt_data.get("Pos")[2].value))[
                                                                              0]]))])
                nbt_block_pos = amulet_nbt.TAG_List([amulet_nbt.TAG_Int(new_pos[0]),
                                                     amulet_nbt.TAG_Int(new_pos[1]),
                                                     amulet_nbt.TAG_Int(new_pos[2])])
...
class EntitiePlugin(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        window = wx.Frame(None, title="The Entitie Plugin", size=(500, 500),style=wx.DEFAULT_FRAME_STYLE | wx.STAY_ON_TOP)
        wx.Panel.__init__(self, window)
        DefaultOperationUI.__init__(self, window, canvas, world, options_path)
...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        window = wx.Frame(None, title="Multi Selection Tool", size=(350, 700), style=wx.DEFAULT_FRAME_STYLE | wx.STAY_ON_TOP)
        wx.Panel.__init__(self, window)
...
class Finder_Replacer(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._is_enabled = True
        self._moving = True
        window = wx.Frame(None, title="Finder Replace", size=(405, 600),
    ...
    def finder(self, look, block=False):
        ...
        if block:
            ...
            self.prog = wx.ProgressDialog("Searching for : " + str(self.textSearch.GetValue()),
                                          ...
                                          style=wx.PD_AUTO_HIDE | wx.PD_CAN_ABORT | wx.PD_ELAPSED_TIME | wx.PD_REMAINING_TIME)
            screen_sise = wx.GetDisplaySize()
            print(screen_sise)
            self.prog.SetPosition(pt=(screen_sise[0]-500,25))
            self.prog.Show(True)
            for x,y,z in look:
                if self.prog.WasCancelled():
                    self.prog.Hide()
...
class ThePlugins(wx.Panel, DefaultOperationUI):
    def __init__(
            self,
            parent: wx.Window,
            canvas: "EditCanvas",
        ...
        self.Freeze()
        self._sizer = wx.BoxSizer(wx.VERTICAL)
        self.test2 = wx.Button(self, label="Entitie Plugin")
        self.test2.Bind(wx.EVT_BUTTON, lambda event: self.load(event, parent, canvas, world, options_path))
        self._sizer.Add(self.test2)
        self.test = wx.Button(self, label="Multi Selection Tool", pos=(90, 0))
        self.test.Bind(wx.EVT_BUTTON, lambda event: self.load2(event, parent, canvas, world, options_path))
        self.test3 = wx.Button(self, label="Finder Replacer", pos=(210, 0))
        self.test3.Bind(wx.EVT_BUTTON, lambda event: self.load3(event, parent, canvas, world, options_path))
        self._sizer.Add(self.test)
        self._sizer.Add(self.test3)
        self.ctrl = EntitiePlugin
        self.is_enty_on = False
        self.Layout()
        self.Thaw()

    def load(self, e, parent, canvas, world, options_path):
        self.ctrl = EntitiePlugin(self, canvas, world, options_path)
        self.is_enty_on = True
        self.bind_events()

    def load2(self, e, parent, canvas, world, options_path):
        SetBlock(self, canvas, world, options_path)

    def load3(self, e, parent, canvas, world, options_path):
        Finder_Replacer(self, canvas, world, options_path)
    ...
    def bind_events(self):
        super().bind_events()
        if self.is_enty_on:
            self.canvas.Bind(EVT_SELECTION_CHANGE, lambda event: self.ctrl._set_new_block(event))
        self._selection = BlockSelectionBehaviour(self.canvas)
        self._selection.bind_events()
        self._selection.enable()
    def enable(self):
...
export = dict(name="PremiereHell's Plugins Windowed", operation=ThePlugins) #PremiereHell